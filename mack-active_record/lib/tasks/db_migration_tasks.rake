...
namespace :db do
  ...
  task :migrate => "mack:environment" do
    ActiveRecord::Migrator.up(File.join(Mack::Configuration.root, "db", "migrations"))
  end # migrate
  
  desc "Rolls the schema back to the previous version. Specify the number of steps with STEP=n"
  task :rollback => ["mack:environment"] do
    ActiveRecord::Migrator.rollback(File.join(Mack::Configuration.root, "db", "migrations"), (ENV["STEP"] || 1).to_i)
  end # rollback
